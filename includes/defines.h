...
/* ---- Resolution bounds for the [width height] arguments ---- */
...
/* ---- Menu ---- */
# define MENU_TITLE         "ft_newton"
# define MENU_NOTE_BACK     "ESC or RESUME to go back"
# define MENU_NOTE_DIRTY    "APPLY to use the edits"
...
# define MENU_PANEL_RATIO   0.75f  /* share of the window width and height the panel covers */
# define MENU_MIN_SCALE     1.0f   /* screen pixels per font pixel, below which text would lose pixels */
# define MENU_MAX_STRETCH   1.5f   /* rows may grow this much taller than the text to fill the panel */
# define MENU_LINE_PIXELS   (FONT_GLYPH_SIZE + 3)  /* font pixels of one menu row */
# define MENU_PAD           6.0f   /* font pixels between the panel edge and its content */
...
# define HUD_REFRESH_PERIOD 0.2f   /* s between two rewrites of the title */
...
/* ---- Meshes and debug display ---- */
# define SPHERE_SEGMENTS     32     /* rings and slices of the sphere mesh */
# define DEBUG_WIRE_INFLATE  1.01f  /* wireframes drawn this much larger than the solid */
# define DEBUG_PLANE_LIFT    0.01f  /* m the plane wireframe floats above the solid */
...
# define FIXED_DT            (1.0f / 120.0f) /* s: fixed physics step, 120 Hz */
# define MAX_STEPS_PER_FRAME 32              /* fixed steps one frame may run; extra simulated time is dropped */
# define SOLVER_ITERATIONS   16              /* contact solver passes per step */
# define ANGULAR_DAMPING     0.98f           /* share of the angular velocity kept per second */
...
# define PENETRATION_PERCENT    0.2f   /* share of the overlap fixed per pass */
...
# define SLEEP_TIME          0.5f    /* s below both thresholds before an island sleeps */
# define WORLD_CULL_DISTANCE 200.0f  /* m from the origin: dynamic bodies beyond are removed */
# define FALL_CULL_DEPTH     20.0f   /* m below a plane: a body that fell off its edge is removed */

/* ---- Collision detection ---- */
# define MAX_CONTACTS_PER_PAIR  4       /* manifold cap per body pair */
# define PLANE_THICKNESS        2.0f    /* m under a plane's surface still solid, so a body that sank in is pushed out */
# define SAT_EDGE_BIAS_REL      0.95f   /* an edge axis must beat a face axis by this ratio ... */
# define SAT_EDGE_BIAS_ABS      0.005f  /* ... and by this many m, or the face wins (steadier manifolds) */
...
# define CAM_MIN_PITCH     DEG2RAD(-89.0f) /* rad: straight down would make the view degenerate */
# define CAM_MAX_PITCH     DEG2RAD(89.0f)  /* rad */

/* ---- Live controls (change per second while a key is held) and their bounds ---- */
...
# define PROJECTILE_MASS_MAX    50.0f    /* kg */
# define GRAVITY_MIN            (-40.0f) /* m/s^2 */
# define GRAVITY_MAX            20.0f    /* m/s^2 */
# define FRICTION_MAX           20.0f    /* the menu's upper bound (>1 is legal, just very grippy) */
# define ROLLING_RESISTANCE_MAX 1.0f     /* rolling resistance coefficient: resisting torque / (normal force * radius) */
...
/* ---- Scene layout ---- */
# define STRUCTURE_X         8.0f   /* m: where 1/2/3 spawn structures */
# define STRESS_X            20.0f  /* m: where 4 spawns the large wall */
# define START_PYRAMID_ROWS  5      /* rows of the pyramid the scene starts with */
# define BLOCK_GAP           1.002f /* stacked blocks start this much apart (share of their size) instead of touching */
# define SPAWN_TOLERANCE     0.001f /* m of overlap still read as touching when a new body is placed */
# define SPAWN_CLEARANCE     0.01f  /* m left under a structure lifted onto what occupies its spot */
